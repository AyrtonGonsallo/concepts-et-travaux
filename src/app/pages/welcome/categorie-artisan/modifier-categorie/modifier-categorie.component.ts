...
export class ModifierCategorieArtisanComponent {
  ...
  submitForm(): void {
    if (this.validateForm.valid) {
      ...
      this.userService.updateCategorieArtisan (parseInt(this.categorieId) ,this.validateForm.value).subscribe(
        (response: any) => {
          console.log('categorie-artisan  modifiée avec succès :', response);
          this.message.create('success', `categorie-artisan  modifiée avec succès`);
          this.router.navigate(['/administration/categories-artisan']);
        },
        (error: any) => {
          console.error('Erreur lors de l\'ajout de l\'categorie-artisan :', error);
        }
      );
    } else {
  ...
  getDetails(id: number): void {
    this.userService.getCategorieArtisanById(id ).subscribe(
      (response) => {
