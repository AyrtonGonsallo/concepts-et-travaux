...
export class ArtisanDetailsComponent {
  ...
  loadModeleEquipements(): Promise<void> {
    return new Promise((resolve, reject) => {
      this.userService.getArtisanModelesEquipement(this.userId).subscribe({
  ...
  exporter() {
    this.userService.getExportModeleEquipementsArtisan(this.userId).subscribe((blob: Blob) => {
  ...
  telecharger_modele() {
    this.userService.downloadModelesArtisansEquipements(this.userId).subscribe((blob: Blob) => {
      // Créer une URL temporaire pour le fichier
      const url = window.URL.createObjectURL(blob);
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `modele_import_equipements_artisan_${this.userId}.xlsx`;  // Nom du fichier
 ...
 loadGamme(): Promise<void> {
  ...
  telecharger_modeleTaches() {
    this.userService.downloadModeleGammesArtisan(this.userId).subscribe((blob: Blob) => {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `modele_import_taches_artisan_${this.userId}.xlsx`;  // Nom du fichier
