...
@Component({
  selector: 'app-modifier-front-page',
  templateUrl: './modifier-front-page.component.html',
  styleUrl: './modifier-front-page.component.css'
})
export class ModifierFrontPageComponent {
  ...
  pageId:string =  this.route.snapshot.paramMap.get('id')??'0';
  submitForm(): void {
    ...
    if (this.validateForm.valid) {
      ...
      this.userService.updateFrontPage(parseInt(this.pageId) ,this.validateForm.value).subscribe(
        (response: any) => {
  ...
  ngOnInit(): void {
    this.getDetails(parseInt(this.pageId, 10))
  }
  ...
  getDetails(id: number): void {
    this.userService.getFrontPageById(id ).subscribe(
      (response) => {
        let keywords= response.Content_balise_keywords.split(",");
        for (let index in keywords) {
          this.tags.push(keywords[index] );
        }
